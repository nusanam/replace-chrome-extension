...
$(function(){
    chrome.storage.sync.get('replacement', function(string){
        $('#replacement').val(string);
    });

    $('#saveReplacement').click(function(){
        var replacedWord = $('#replacement').val();
        if (string){
            chrome.storage.sync.set({'string': string}, function(){
                close();
            });
        }
    });
    ...
    $('#resetWords').click(function(){
        chrome.storage.sync.set({'total': 0}, function(){
            ...
            var notifOptions = {
                type: "basic",
                title: "Resetting Custom Words",
                message: "Custom words to replace have been removed."
            };
